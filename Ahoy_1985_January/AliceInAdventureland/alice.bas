...
2880 RETURN
2900 POKEV+28,2:POKEV+39,8:POKE2040,15
2910 S(0)=1:POKEV+21,FNR(R):POKEV+1,PY(5):POKEV,PX(5):GOSUB600:SK=-1
2920 FORI=1TO3:F(I)=VAL(MID$(D$(Z+2),I,1)):NEXT
2930 DR=(F(1)<3ANDF(2)<3)+(F(1)>2ANDF(2)<3)*2+(F(1)<3ANDF(2)>2)*3
2935 DR=DR+4*(F(1)>2ANDF(2)>2)
2950 IFS(Z+2)>0THENPRINTSP$"'I SEE YOU HAVE THE [c 8]"K$(Z+7)"[CYAN]!'":RETURN
2952 PRINTSP$"YOU'LL FIND THE [c 8]"K$(Z+7)
2955 PRINTSP$"[DOWN]IN THE [YELLOW]"DR$(ABS(DR))"[CYAN] SECTION ON LEVEL[YELLOW]"STR$(F(3)+1)"[CYAN]'":RETURN
3100 PRINTE$(E-16):E=0:RETURN
3200 PRINTE$(E-16):SK=-1:GOSUB600:IFGS>0THENPRINTSP$;ST$(E-16)
3202 E=0:IFGS=0THENPRINTSP$"SORRY, LITTLE GIRL, BUT YOU NEED GOLD":RETURN
3205 POKEG2+GS,32:GS=GS-1:GOSUB2640:RETURN
3300 POKEG1,81:POKEG1+LC,7:GOSUB600:PRINTSP$"YOU FOUND A GOLD COIN!":SK=-1 
3310 GS=GS+ABS(GS>-1):POKEG1,32:POKEG2+GS,81:POKEG2+GS+LC,7:GOSUB1900
3320 R$(X,Y,Z)="0"+RIGHT$(R$(X,Y,Z),4)
3330 IFGS<10THEN:RETURN
